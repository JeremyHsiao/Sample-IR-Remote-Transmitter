 ...
 * $Revision: 1 $
 * $Date: 15/09/24 1:00p $
 * @brief    N575 WDT driver source file
 *
 * @note
...
/** @addtogroup N575_WDT_Driver WDT Driver
  @{
*/


/** @addtogroup N575_WDT_EXPORTED_FUNCTIONS WDT Exported Functions
  @{
*/
...
void  WDT_Open( uint32_t u32TimeoutInterval, uint32_t u32EnableReset )
{
    ...
    WDT->CTL = u32TimeoutInterval | WDT_CTL_WDTEN_Msk | WDT_CTL_INTEN_Msk | (u32EnableReset << WDT_CTL_RSTEN_Pos) ;
	SYS_Lock(u8Lock);
}
...
/*@}*/ /* end of group N575_WDT_EXPORTED_FUNCTIONS */

/*@}*/ /* end of group N575_WDT_Driver */
...
//
// Here is the modified function created by Jeremy Hsiao
//

void WDT_MySetup(void)
{
	uint8_t u8Lock = SYS_Unlock();
    WDT->CTL = ((0x06)<<WDT_CTL_TOUTSEL_Pos) | WDT_CTL_WDTEN_Msk | WDT_CTL_INTEN_Msk | WDT_CTL_RSTEN_Msk |  WDT_CTL_RSTCNT_Msk;
	SYS_Lock(u8Lock);
}

void WDT_MyClearTimeOutIntFlag(void)
{
	uint8_t u8Lock = SYS_Unlock();	
	WDT->CTL = (WDT->CTL & ~WDT_CTL_RSTF_Msk) | WDT_CTL_IF_Msk;	
	SYS_Lock(u8Lock);
