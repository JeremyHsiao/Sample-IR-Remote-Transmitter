 ...
 * $Revision: 1 $
 * $Date: 15/09/24 2:52p $
 * @brief    N575 series PWM driver header file
 *
 * @note
...
/** @addtogroup N575_PWM_EXPORTED_CONSTANTS PWM Exported Constants
  @{
*/
	
#define PWM_CHANNEL_NUM                     (2)   /*!< PWM channel number */
...
/*@}*/ /* end of group N575_PWM_EXPORTED_CONSTANTS */
...
#define PWM_ENABLE_OUTPUT_INVERTER(pwm, u32ChannelMask) \
    { \
        int i;\
                (pwm)->CTL &= ~(PWM_CTL_PINV0_Msk|PWM_CTL_PINV1_Msk);\
...
void PWM_ConfigOutputChannel(PWM_T *pwm,
                                 uint32_t u32ChannelNum,
                                 uint32_t u32Frequncy,
                                 uint32_t u32DutyCycle);
